...
# Import the required packages for statsmodels
import statsmodels.api as sm
from statsmodels.tsa.arima.model import ARIMA

#os.environ["GOOGLE_APPLICATION_CREDENTIALS"]="lstm-gcloud-project-2e518442c03c.json"
...
def forecast():
    ...
    # data_frame[type] = pd.to_datetime(data_frame[type])
    # data_frame = data_frame.set_index(type)
    # df1 = data_frame.groupby(pd.Grouper(freq="d"))['issue_number'].count()
    # df1 = df1.rename_axis(type).reset_index()
    ...
    BASE_IMAGE_PATH = os.environ.get(
        'BASE_IMAGE_PATH', 'https://storage.googleapis.com/lstm-unique-cloud-bucket/')
    ...
    BUCKET_NAME = os.environ.get(
        'BUCKET_NAME', 'lstm-unique-cloud-bucket')
    ...
    # implementation using FB Prophet
    # setting the image url for the cloud to store the plot
    ...
    PROPHET_FORECASTED_DATA_IMAGE_NAME = "prophet_forecast_data_" + type + "_"+ repo_name + ".png"
    PROPHET_FORECASTED_DATA_URL = BASE_IMAGE_PATH + PROPHET_FORECASTED_DATA_IMAGE_NAME

    #model for Forecasting (using the prophet library)
    my_model = Prophet(interval_width=0.95,changepoint_prior_scale=0.1, daily_seasonality=True)
    my_model.fit(df)

    # create the df for future dates (daily for the next year)
    future_dates = my_model.make_future_dataframe(periods=365, freq='D',include_history=False)

    forecast = my_model.predict(future_dates)
    #my_model.plot(forecast, uncertainty=True) # plotting the forecasted values against the timestamp
    my_model.plot_components(forecast).savefig(LOCAL_IMAGE_PATH+PROPHET_FORECASTED_DATA_IMAGE_NAME)


    #Implementing using StatsModels
    # setting the image url for the cloud to store the plot
    STATSMODELS_FORECASTED_DATA_IMAGE_NAME = "statsmodels_forecast_data_" + type + "_"+ repo_name + ".png"
    STATSMODELS_FORECASTED_DATA_URL = BASE_IMAGE_PATH + STATSMODELS_FORECASTED_DATA_IMAGE_NAME

    tS = df.set_index('ds').asfreq('d')

    mod = ARIMA(tS, order=(7, 0, 2)) # using ARIMA Model for forecasting values
    mod_fit = mod.fit()

    print(mod_fit.summary())

    #ftr =  (tS.index + pd.Timedelta(365, unit='days')).to_frame() # creating the future dataframe (for 1 year, 365 daYs) for forecasting

    forecast = mod_fit.forecast(364, alpha = .05) #forecast values to 'future' dataframe for future 361 (# of daYs) steps
    forecast = pd.DataFrame(forecast)
    forecast.plot(label='Forecast').get_figure().savefig(LOCAL_IMAGE_PATH+STATSMODELS_FORECASTED_DATA_IMAGE_NAME) #plotting the forecasted values against the timestamp
    ...
    new_blob = bucket.blob(PROPHET_FORECASTED_DATA_IMAGE_NAME)
    new_blob.upload_from_filename(
         filename=LOCAL_IMAGE_PATH + PROPHET_FORECASTED_DATA_IMAGE_NAME)

    new_blob = bucket.blob(STATSMODELS_FORECASTED_DATA_IMAGE_NAME)
    new_blob.upload_from_filename(
         filename=LOCAL_IMAGE_PATH + STATSMODELS_FORECASTED_DATA_IMAGE_NAME)
    ...
    json_response = {
        ...
        "lstm_generated_image_url": LSTM_GENERATED_URL,
         ...
         "prophet_forecast_data_image": PROPHET_FORECASTED_DATA_URL,
         "statsmodels_forecast_data_image": STATSMODELS_FORECASTED_DATA_URL
     }
